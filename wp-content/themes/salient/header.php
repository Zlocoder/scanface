<?php
    ...
    if (!isset($_SESSION['facecheck_user'])) {
        if (is_page_template('result.php')) {
            $_SESSION['redirect_result_id'] = get_query_var('facecheck_photo_id');
        } elseif (!is_page_template('login.php') && !is_page_template('registration.php')) {
            unset($_SESSION['redirect_result_id']);
        }
    }
?>
...
<head <?php language_attributes(); ?>>
    ...
    <?/*php if (is_page_template('upload.php') || is_page_template('markers.php') || is_page_template('result.php') || is_page_template('history.php')) { ?>
        <meta name="viewport" content="width=1175" />
    <?php } else { ?>
    ...
    <script type="text/javascript">
        jQuery(function() {
            jQuery(document).enableTouch();
            jQuery('a.video').magnificPopup({type: 'iframe'});
        });
    </script>
</head>
			...
			<div class="row">
				...
				<div class="col span_9 col_last">
                    ...
                    <?php if(has_nav_menu('top_nav')) : ?>
                        <nav>
                            <ul class="sf-menu">
                                <?php wp_nav_menu( array('walker' => new Arrow_Walker_Nav_Menu, 'theme_location' => 'top_nav', 'container' => '', 'items_wrap' => '%3$s' ) ); ?>
                    ...
                    <div class="buttons">
                        <?php if ($headerBlue) { ?>
                            <?php if(isset($_SESSION['facecheck_user'])) { ?>
                                <span>Добро пожаловать,<br/><?=$_SESSION['facecheck_user']->name?></span>
                                <a class="nectar-button extra-color-1" href="?action=facecheck_user_logout&redirect=<?= get_permalink(get_page_by_title('Вход')) ?>">Выход</a>
                            <?php } else { ?>
                                <span>Добро пожаловать,<br/>Гость</span>
                                <a class="nectar-button extra-color-1" href="<?=get_permalink(get_page_by_title('Вход'))?>">Вход</a>
                            <?php } ?>
                        <?php } elseif(is_page('Вход')) { ?>
                            <span>Еще не зарегестрированы?</span>
                            <a class="nectar-button extra-color-2" href="<?=get_permalink(get_page_by_title('Регистрация'))?>">Регистрация</a>
                        <?php } elseif(is_page('Регистрация')) { ?>
                            <span>Уже зарегистрированы?</span>
                            <a class="nectar-button extra-color-2" href="<?=get_permalink(get_page_by_title('Вход'))?>">Login</a>
                        <?php } else { ?>
                            <?php if(isset($_SESSION['facecheck_user'])) { ?>
                                <a class="nectar-button accent-color" href="<?=get_permalink(get_page_by_title('History'))?>">Кабинет</a>
                                <a class="nectar-button extra-color-2" href="<?= '?action=facecheck_user_logout&redirect='.get_permalink(get_page_by_title('Вход'))?>">Выход</a>
                            <?php } else { ?>
                                <a class="nectar-button accent-color" href="<?=get_permalink(get_page_by_title('Upload'))?>">Начать</a>
                                <a class="nectar-button extra-color-2" href="<?=get_permalink(get_page_by_title('Вход'))?>">Login</a>
                            <?php } ?>
                        <?php } ?>
                    </div>
